...
参考：https://blog.csdn.net/hxhxhxhxx/article/details/108111692
...
我们需要要让ic返回值为true，而token里要填的是a数列里的每位数减3，转换ascii码得到的字符，输入后得到flag
...
流量分析题，我们先拿modbus过滤一下，再追踪TCP流，发现flag

![image-20210802104054360](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210802104054360.png)



### wireshark-1

这道题需要我们通过流量包分析出登录的密码，所以首先我们需要找到登录页面的数据包

过滤一下http包，发现我们要找的数据包

![image-20210802112213390](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210802112213390.png)

查看数据包发现密码

![image-20210802112300622](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210802112300622.png)



### pure_color

用stegsolve在blue plane 0可以看到flag

![image-20210802152139685](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210802152139685.png)



### Aesop_secret

首先观察，发现gif重复闪烁，注意到这个gif文件播放时每一帧的位置都不一样，想到如果把每一帧的图像同时显现出来会怎么样。于是我们用Photoshop打开这个文件，显现每一帧图像。

显示图案字母：**ISCC**

![image-20210802154155625](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210802154155625.png)

binwalk分解未发现，但winhex发现末尾字符，复制出来

```
U2FsdGVkX19QwGkcgD0fTjZxgijRzQOGbCWALh4sRDec2w6xsY/ux53Vuj/AMZBDJ87qyZL5kAf1fmAH4Oe13Iu435bfRBuZgHpnRjTBn5+xsDHONiR3t0+Oa8yG/tOKJMNUauedvMyN4v4QKiFunw==
```

![image-20210802154317579](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210802154317579.png)

一开始以为是base64，尝试无果，想到之前的ISCC是不是密钥，于是尝试带密钥的AES加解密

[AES加解密网站](https://www.sojson.com/encrypt_aes.html)

这里要两次AES解密

![image-20210802154851978](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210802154851978.png)



### a_good_idea

改个后缀就可以看到压缩包的其他图片了，题目提示我们查看像素，两张图片我们就可以进行诸如异或之类的操作了

最后将a_very_good_idea.jpg和to.png进行叠加，最后ps调整一下杂色就可以拿去扫了

![image-20210802161755378](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210802161755378.png)
...
题目描述给出了一串字符串，其中"-"肯定是分隔，每段字符的数量又都是2的倍数，且一共有12行
...
pdf是新加坡2017年的日历，推理之后有理由怀疑字符对应的是日期，一行对应一个月，每两位代表一个日期

![20210802120055330](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/20210802120055330.png)
...
拿binwalk分解一下，发现一个加密的zip，爆破一下密码为fish



### Test-flag-please-ignore

winhex打开，发现16进制编码字符串，解码得到flag



### hit-the-core

winhex打开，观察字符，发现有一段比较可疑，其中每隔四个是大写的字母，我们把这一段抠出来

![20210802150238430](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/20210802150238430.png)
...
发现像gif把图片一条的切了出来，我们需要先将gif分帧，我这里用到GIFFrame，也可以写一个脚本，接下来就是合并
...
拿到一张图片去binwalk一下，得到一个加密的rar

分析这个png图片，通过更改png图片高度来得到被隐藏的密码

> Tip：第一排16进制是固定的，这里00 00 03 9E 是宽度，00 00 04 4C是高度

![image-20210802174931673](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210802174931673.png)

![image-20210802175223106](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210802175223106.png)

得到rar中的流量包

流量太多了，常规做法是搜索字符串png，发现一个kiss.png，发现base64后的flag

![image-20210802175554656](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210802175554656.png)
...
直接追踪TCP流就好了，flag格式有点坑，需要直接提交这个
...
丢进Stegsolve中进行查看，发现有一张二维码，具体如下所示

![image-20210803095809790](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210803095809790.png)

扫描后发现是pyc的文件，文件头为03F30D0A，这个是.pyc文件（反编译）

![image-20210803095846067](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210803095846067.png)

再Easy Python Decompiler反编译，打开后，得到源码
...
在linux中，删除文件时其实删的是文件名，数据还是存储在硬盘中的

我们先安装extundelete `sudo apt-get install extundelete`
...
![image-20210803104548479](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210803104548479.png)

完成后多了一个文件夹，运行一下flag

![image-20210803104559816](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210803104559816.png)
...
我们调整通道，发现Red为0时为一张jpeg，我们Save Bin，由图片得到flag

![image-20210803105533152](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210803105533152.png)
...
binwalk跑一下，分解出一个加密zip，一开始以为密码是图片上的md5，失败。看提示

> 假如你是李华（LiHua)，收到乔帮主一封密信，没有任何特殊字符，请输入密码，不少于1000个字。同学，记得署名哦～

这里的1000如果作为二进制，就是8，所以密码不小于8位，可8位要爆破到什么时候？

`记得署名哦？`，会不会是最后是LiHua，xxxLiHua

那就用AZPR爆破

![image-20210803151901098](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210803151901098.png)

最后密码为`15CCLiHua`，得到flag



### embarrass

以为是道流量分析题，结果winhex找一下flag就可以看到了



### MISCall

由于不知道是什么类型的文件，就用kali跑一下
...
![image-20210803153738190](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210803153738190.png)
...
![image-20210803154508418](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210803154508418.png)

解压后会有一个ctf文件夹，里面有个flag.txt，点开啥都没有，但是我们可以看到有很多`.git`内容

这里就考到了 `git文件泄露、git stash命令`
git stash命令可参考https://www.cnblogs.com/zndxall/archive/2018/09/04/9586088.html

利用命令`git stash show` 显示做了哪些改动

![image-20210803154219398](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210803154219398.png)

发现一个s.py文件，利用命令`git stash apply`导出来，运行s.py就能得到flag

![image-20210803154238527](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210803154238527.png)
...
![流2](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/20201126184902295.png)

继续往后翻，在第三个流里面发现一个png文件，在下面选择原始数据，保存。

![流3](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/20201126185109364.png)

继续翻，在第七个流里面发现一段base64文本，解码后是一个假flag

![流7](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/20201126190303280.png)

全都保存出来后得到了这么几个文件：

![提取出的文件](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/20201126191044873.png)

逐个检查这几张图片，发现在最后一张里存在LSB隐写，用Stegsolve打开文件，一番操作之后flag就出来了

![Stegsolve](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/20201126192552494.png)
...
![image-20210803170717522](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210803170717522.png)
...
![image-20210803170932282](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210803170932282.png)

在1这个文件中找到了，看看这个文件的类型：

![image-20210803170951815](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210803170951815.png)

这里用普通的解压是不行的，需要用命令：

> 用gunzip的话，后缀不正确，要是把后缀改成gz，解压后仍是文件1
> 用tar的话，这个文件不是标准的gzip文件，编码应该是有问题的
> 用binwalk检测一下，什么也没有
...
拿到flag：SECCON{@]NL7n+-s75FrET]vU=7Z}
...
![在这里插入图片描述](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/20200105141916508.png)
...
复制出来得到一张倒过来的图片，镜像一下就好了

![202108031723](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/202108031723.jpg)
...
这道题容易想复杂，直接winhex查一下flag

![image-20210803173030238](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210803173030238.png)
...
![image-20210803173853625](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210803173853625.png)
...
再注意绿色数字：`0405111218192526`，这个就是密码了

![image-20210803185630744](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210803185630744.png)

解出一个try.zip文件，解压要密码，看提示是一个生日日期

![image-20210803185728324](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210803185728324.png)
...
被加密的压缩包中含有已知文件，很明显明文攻击。把readme.txt压缩成zip然后使用ARCHPR执行明文攻击

![image-20210803185916164](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210803185916164.png)

```
恢复解压密码：Y29mZmVl
```

最后是一个伪加密，用工具ZipCenOp

```
java -jar ZipCenOp.jar r flag.zip
```

解压后打开flag.txt，凯撒一下，得到flag



### 倒立屋

受之前那个“打野”影响，直接丢到zsteg解，发现字符

![image-20210803190547893](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210803190547893.png)

提交还需要将字符反转，zsteg真是神器
...
打开之后甚至以为是什么密码，但是看到最后 This is not program 想到是不是此地无银三百两这样的，于是搜了搜rockstar语言，真的搜到了

github上有转换方法，`pip install rockstar-py`
...
打开流量包发现1，0之类的，猜测二进制转ASCII字符串

![image-20210804105820188](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210804105820188.png)

提取，发现不正确

```
10110111 10011010 10010110 10001100 10011010 10010001 10011101 10011010 10001101 10011000
```
...
![img](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/1785869-20210628160716856-2077336623.png)

等待安装完成后，把数据包拖进去，输入命令拆解一下看看

```
tcpxtract -f 434c8c0ba659476caa9635b97f95600c.pcap
```

![img](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/1785869-20210628160847073-923446265.png)

得到两张图片，在Kali打开好像有点问题，拖到Windows系统里打开，发现flag

![img](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/1785869-20210628160954412-1367745195.png)



### simple_transfer

可以在协议分级中看到，协议的字节百分比占用基本全在NFS协议上，直接过滤NFS

观察报文，发现有个pdf文件

![在这里插入图片描述](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/20201111203328692.png)

直接foremost分解，得到pdf文件，打开后里面什么都没有，直接CTRL + A 全选复制出flag



### Erik-Baleog-and-Olaf

直接foremost分解一下，得到一个png，用stegsolve调整通道发现一个二维码

![image-20210804152109175](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210804152109175.png)
...
winhex查看文件数据，发现是**gpx**数据，也就是gps的，到相应网址解析

> https://www.gpsvisualizer.com/map_input

上传文件到平台，获取到地图

![在这里插入图片描述](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/20210524120754283.png)

选择风景浏览，就是gps地图会显示出地名

![在这里插入图片描述](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/20210524121106245.png)

接下来就是按照顺序依次找到每个地点的英文排列下来。
...
![image-20210804153617175](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210804153617175.png)

winhex打开压缩包内文件，搜索ctf发现flag



### low

根据提示low可以是LSB隐写，运行代码即可获取二维码
...
Linux下使用命令，即查找webshell中的带有“{”的ASCII字符串

```
strings webshell.pcapng | grep {
```

![image-20210804162202576](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210804162202576.png)

**预期解**

进行一个流量分析

- `ip.addr == 192.168.1.0/24`过滤内网ip
- 192.168.1.111大量访问外网，猜测是192.168.1.108做反向代理攻击
- 过滤192.168.1.108追踪tcp流得到flag

```
shell s@.(.N`k.UuUI.D.......o.....c...i.H.m..j...x....t.... ?.].b......9.;.....n5...f]$.6..r....0.b.......D,.*.WMa.B.....+a<....bh.............&..<.~8.hgvq;.N..w.....q;.N.v...L.;.6i.*.N..Ls.....q;.N.......i....c...fl4g:{ftop_Is_Waiting_4_y}
```
...
视频题，用MKVToolnixPortable提取出两个音频

![20210804125914352](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/20201207125914352.png)

然后扔进audacity，查看频谱图

![20210804130007455](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/20201207130007455.png)
...
WP是把这25个图片按5×5排列，然后把有数字的格记为数字1(代表黑色)，没有的记为0(代表白色)，再把得到的数字用python画出二维码

仔细观察发现，其实第1、5、21这三张图的位置错了，我们需要将第21张换到第1张的位置，第1张换到第5张的位置，第五章换到第21张的位置
...
二进制位，8个二进制转成一个字符。这里只有105个，前七位1100110，**是f的二进制形式。**所以需要取出凑够8位，将步长设置成7，然后 + 0，再转成字符串，一气呵成即可！

最后打印出flag{W0W*funny}

![image-20210804184343403](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210804184343403.png)
...
先进行tcp连接，再是FTP传输，应该是一个在传输某些文档的流量包，推测flag应该和传输的文档有关。

追踪tcp流，第二个流时发现有一些东西，pub.key引起重视，并且很有可能有私钥，推测会用到私钥加密。并继续追踪。

成功找到私钥，复制处理，重命名为rsa.key

![image-20210804185108560](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210804185108560.png)

我们把流量包扔进foremost进行分离，发现被加密的txt文件，放到kali的openssl进行解密
...
![image-20210804185658825](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210804185658825.png)
...
可以用potplayer翻找，在第57帧发现，果断使用ASII进行编码

![image-20210804190042329](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210804190042329.png)
...
先拿foremost分解一下，得到一个压缩包和一个图片

我们来看buguoruci.zip下的 chayidian.jpg ，分解图片zip后发现是假的flag，直接暴力破解chadiand.zip

![chayidian](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/chayidian.jpg)

但是，查看两个文件的CRC32 可知两个文件一样，很明显这是一个明文攻击，我们用AZPR对比爆破

最后跑出密码为：`z$^58a4w`，拿着密码将加密文件 flag.zip解压，得到如下几个文件

![image-20210804192916759](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210804192916759.png)

- 打开whoami.zip文件，发现有个加密文本，需要密码，猜想flag就在里面

- 打开world.doc文件，发现了隐藏字符

  ![20210804203846526](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/20210804203846526.png)

- 打开 world/media/task/writeup/cn/miscmisc/1.png图片，猜测是图片隐写，最后在LSB BGR条件下发现pass

  ![20210804203846536](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/20210804203846536.png)

得到pass：z^ea，但去解压文件 发现不行，其实最后的密码是**pass内容+world里每行字符串的最后一个字符**，即`z^ea4zaa3azf8`，最后拿到flag



### 奇怪的TTL字段

文本打开，TTL有29万行，但观察后发现一共只有四种TTL值，可以看出后面六位全是1，只有前面两位藏了数据，也就是说一组TTL值可以隐藏一个字节
...
参考大佬写的脚本，最后跑出来一张图片，jpg格式是首先计算前32位二进制位判断出来的
...
用foremost分离出六张二维码片段，后面用PS把图片重新拼接，然后二维码识别，得到

```
key:AutomaticKey cipher:fftu{2028mb39927wn1f96o6e12z03j58002p}
```

根据key的信息，可以联想到一种常用的加密，Automatic加密，解密网站：https://www.wishingstarmoye.com/ctf/autokey

![image-20210804195154132](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210804195154132.png)



### Miscellaneous-300

拿到一个加密压缩包，爆破密码为46783，得到46783.zip，发现密码就是压缩包内文件名

接着又是一个加密压缩包，尝试文件名，成功
...
最后会有12475.zip，我们爆破密码b0yzz，给的是音频文件，猜测是音频隐写，于是将文件拖入Audacity中查看频谱图得到flag
...
我们winhex打开图片发现文件头不一样，修改为正确的之后还是打不开，发现宽度也错了，宽度要改为符合crc的宽度
...
得到宽度为709，转为16进制为02C5，最终改为

![image-20210804213728257](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210804213728257.png)



### 3-11

用 Steg­solve 打开，发现是 LSB 隐写，开头 PK, zip 的文件头

![image-20210805155545891](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210805155545891.png)



打开压缩包里的flag.txt，看开头iVBORw0KGgoAAA......这是base64编码的图片, 用工具转成png图片即可

解码网站：https://tool.jisuapi.com/base642pic.html

![image-20210805155626849](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210805155626849.png)

得到flag：FLAG{LSB_i5_SO_EASY}

![3005662702](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/3005662702.png)
...
![image-20210805161843513](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210805161843513.png)

发现是一个数据包，用wireshark打开查看，追踪了下TCP流，发现存有多张图片。利用导出http对象，将所有图片导出

![image-20210805161914662](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210805161914662.png)

发现其中一张有二维码，扫描后发现

![image-20210805162229151](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210805162229151.png)

```
U2FsdGVkX1+VpmdLwwhbyNU80MDlK+8t61sewce2qCVztitDMKpQ4fUl5nsAZOI7bE9uL8lW/KLfbs33aC1XXw==
```

图片提示AES加密，并且`CTF`作为密码进行解密，得到`668b13e0b0fc0944daf4c223b9831e49`

![image-20210805162932150](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210805162932150.png)

这里基本结束，估计别的图片还有信息，根据题目提示，直接找到这张

![image-20210805163050184](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210805163050184.png)

发现这张图片其实是个zip，输入刚刚获取的密码之后，得到二维码，扫描里面那个小二维码得到flag

![a1d63bb3ed1f9df89b72375f1ed79e5d](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/a1d63bb3ed1f9df89b72375f1ed79e5d.jpg)
...
需要使用Logic软件打开，https://www.filetypeadvisor.com/zh-cn/extension/logicdata
...
Ariana Grande---->频谱图—>采样率—>其他

![在这里插入图片描述](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/c3ffb424d210dc5ea4abb8313cba8ab8.JPEG)

**设定采样率—>900—>确定，拖动滚动条到右边**

![image-20210805190221466](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210805190221466.png)



### Excaliflag

用 Steg­solve 打开，调整通道再Blue plane0看到flag

![image-20210805190657574](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210805190657574.png)
...
![20201009235815786](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/20201009235815786.png)



### 信号不好先挂了

首先用 Steg­solve 打开，发现为LSB，分离出zip文件

![image-20210805191730053](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210805191730053.png)

分离出一张一模一样的图，解压出的pen.png再用stegsolve打开，发现**有条纹**，怀疑是盲水印

于是和之前的apple.png解[盲水印](https://github.com/chishaxie/BlindWaterMark)

```
python3 bwmforpy3.py decode apple.png pen.png apple_pen.png --oldseed
```
...
这里我使用的是PotPlayer分帧查看视频，快捷键d、f

![image-20210805201432709](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210805201432709.png)

这个二维码是`i`，还需要继续找，最后一张二维码找了好久死活找不到，调亮了屏幕之后就立马找到了

![image-20210805201951517](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210805201951517.png)

找到所有的密码：`iwantplayctf`，打开压缩包得到flag
...
![image-20210805210942572](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210805210942572.png)

感觉像摩尔斯，手动搞出来

```
0   0111010010101010011000100011101001010101001100010
```

查了下题目提示，[使用HackCube-Special分析固定码信号](http://www.kb-iot.com/post/756.html)

> 钥匙信号(PT224X) = 同步引导码(8bit) + 地址位(20bit) + 数据位(4bit) + 停止码(1bit)

所以flag是`sctf{01110100101010100110}`



### hong

一开始以为是mp3，没用，最后用foremost分离一下文件，得到两张图片，其中一个是flag



### 3-1

下下来一个无后缀文件，丢到winhex，发现是个rar，改后缀解压

扔进winhex发现是个流量包，直接导出http对象，看到flag.rar

![image-20210805215133524](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210805215133524.png)

解压要密码，那么密码还在流量包里

在第6个TCP流中发现一些linux指令，一个base64，一段python

![image-20210805215506341](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/image-20210805215506341.png)
...
得到密码`passwd={No_One_Can_Decrypt_Me}`，解压

```
WDCTF{Sealab_CTF_2017}
```
...
foremost分解一下看到两张图片，盲水印解一下就有flag
...
一看就是古典密码，我们这里要词频分析一下，地址：https://quipqiup.com/



### 5-1

未知类型文件，用[xortool](https://github.com/hellman/xortool)尝试，得到key：`GoodLuckToYou`

![2bb2eaf913e42f868e3d0843d94fa8d9](https://antlersmaskdown.oss-cn-hangzhou.aliyuncs.com/2bb2eaf913e42f868e3d0843d94fa8d9.jpg)
...
这里需要无线电工具，multimon-ng是一个专门解码aprs、psk等业余无线电数字通信协议的工具
